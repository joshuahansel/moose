...
This control implements the classic PID controller, which takes as its input
...
time step, so you should set [!param](/ControlLogic/PIDControl/execute_on) accordingly,
...
- $x$ is set with [!param](/ControlLogic/PIDControl/input).
- $\bar{x}$ is set with [!param](/ControlLogic/PIDControl/set_point).
- $y$ is declared with the name `<control_name>:output`, where `<control_name>`
...
!alert note
To control a controllable value directly instead of a [ControlData.md], use [PIDTransientControl.md].

...
public:
   ...
   */
  bool hasComponent(const std::string & name) const { return _thm_app.hasComponent(name); }
   ...
   * Gets the log
   */
  Logger & log() const { return _thm_app.log(); }
...
{
  return _thm_app.hasComponentOfType<T>(name);
}
...
{
  return _thm_app.getComponentByName<T>(name);
}

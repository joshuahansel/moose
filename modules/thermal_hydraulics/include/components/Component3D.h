...
class Component3D : public GeneratedMeshComponent
{
public:
  ...
  Component3D(const InputParameters & params);
   ...
   */
  unsigned int getNumRadialRegions() const { return _n_radial_regions; }
   ...
   */
  bool hasRadialRegion(const std::string & name) const;

  /**
   * Gets the names of the transverse regions
   */
  const std::vector<std::string> & getRadialRegionNames() const { return _radial_region_names; }

  /**
   * Gets the volumes of the transverse regions
   */
  const std::vector<Real> & getRadialRegionVolumes() const { return _radial_region_volumes; }
...
protected:
  /**
   * Builds a 3D, first-order mesh
   */
  void build3DMesh();
  std::vector<std::vector<std::vector<unsigned int>>> build3DMeshNodes();
  void build3DMeshElems(const std::vector<std::vector<std::vector<unsigned int>>> & node_ids);
  ...
  unsigned int _n_radial_regions;
  /// Names of each transverse region
  std::vector<std::string> _radial_region_names;
  /// Width of each transverse region
  std::vector<Real> _radial_region_widths;
  /// Total width of all transverse regions
  Real _total_width;
  /// Volume of each transverse region
  std::vector<Real> _radial_region_volumes;
  /// Number of elements in each transverse region
  std::vector<unsigned int> _n_radial_elems;
  /// Total number of transverse elements
  unsigned int _total_radial_elems;
  std::vector<Real> _radial_positions;
  ...
  const unsigned int _n_azimuthal_elems;
  const Real _azimuthal_elem_width;
...
// {
// template <>
// Component3D::ExternalBoundaryType stringToEnum(const std::string & s);
// }

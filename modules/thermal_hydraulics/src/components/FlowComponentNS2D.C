...
FlowComponentNS2D::validParams()
{
  ...
  params.addRequiredParam<std::vector<std::string>>("region_names", "Name of each radial region");
  params.addRequiredParam<std::vector<Real>>("region_widths", "Width of each radial region [m]");
  params.addRequiredParam<std::vector<unsigned int>>("region_n_elems",
                                                     "Number of elements in each radial region");
...
FlowComponentNS2D::check() const
{
  ...
  if (isCylindrical())
  {
    if (isParamSetByUser("depth"))
      logError("If the component is cylindrical, then 'depth' may not be provided.");
    if (!isParamSetByUser("inner_radius"))
  ...
  else
  {
    ...
    if (isParamSetByUser("inner_radius"))
      logError("If the component is not cylindrical, then 'inner_radius' may not be provided.");
  }
}

std::vector<std::string>
FlowComponentNS2D::getRegionNames() const
{
  return getParam<std::vector<std::string>>("region_names");
}

std::vector<Real>
FlowComponentNS2D::getRegionWidths() const
{
  return getParam<std::vector<Real>>("region_widths");
}

std::vector<unsigned int>
FlowComponentNS2D::getRegionNumbersOfElements() const
{
  return getParam<std::vector<unsigned int>>("region_n_elems");
}
